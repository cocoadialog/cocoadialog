...
- (instancetype)init {
  ...
  if (self) {
    ...
    options.getOptionOnceCallback = ^(CDOption *opt) {
      ...
      // Note: this must be added here, after available options have populated in
  ...
  if (self) {
    ...
    if (alias) {
      ...
      if (controlNameIndex < self.options.arguments.count) {
        self.options.arguments[controlNameIndex] = alias.controlName;
      }
...
+ (CDOptions *)availableOptions {
  return [CDOptions options].addOptionsToScope(@"global",
    @[
      // Add hidden development (Xcode) option.
...
- (NSScreen *)getScreen {
  ...
  if (index >= [screens count]) {
    self.terminal.warning(@"Unknown screen index: %@. Using screen where keyboard has focus.", @(index), nil);
