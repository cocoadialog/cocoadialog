...
- (instancetype)init {
  ...
  if (self) {
    ...
    if (!_isCLI || !_environment[@"TERM"] || _environment[@"TERM"].isBlank) {
      ...
      _colors = [self execute:[self which:@"tput"] withArguments:@[@"colors"]].numberValue.unsignedIntegerValue;
      _cols = [self execute:[self which:@"tput"] withArguments:@[@"cols"]].numberValue.unsignedIntegerValue;
    }
...
- (CDTerminal *(^)(NSString *, ...))debug {
  return ^CDTerminal *(NSString *format, ...) {
    // Immediately return current log level is not sufficient.
