...
- (void)controlHasFinished:(NSInteger)button {
  if (self.cancelButton && button == self.cancelButton.integerValue) {
  ...
  if (self.options[@"return-labels"].wasProvided) {
    if (button == 0) {
      ...
      buttonValue = self.button1.title;
    }
    else if (button == 2) {
      buttonValue = self.button2.title;
    }
  }
...
- (void)createPanel {
  ...
  if (self.options[@"width"].wasProvided) {
    width = self.options[@"width"].isPercent ? [self.options[@"width"] percentageOf:(float) screen.frame.size.width] : self.options[@"width"].floatValue;
    if (width != 0.0) {
      size.width = width;
    }
  }
  if (self.options[@"height"].wasProvided) {
    height = self.options[@"height"].isPercent ? [self.options[@"height"] percentageOf:(float) screen.frame.size.height] : self.options[@"height"].floatValue;
    if (height != 0.0) {
      size.height = height;
    }
  }
  ...
  CGFloat screenHeight = NSHeight(screen.visibleFrame);
  CGFloat screenWidth = NSWidth(screen.visibleFrame);
  ...
  CGFloat left = x + ((screenWidth - NSWidth(self.panel.frame)) / 2.0f - padding);
  CGFloat top = (y + screenHeight) - (screenHeight / 1.8f) - (NSHeight(self.panel.frame) / 1.8f);

  CGFloat posXNumber = self.options[@"posX"].isPercent ? [self.options[@"posX"] percentageOf:(float) (screenWidth - NSWidth(self.panel.frame))] : self.options[@"posX"].floatValue;
  CGFloat posYNumber = self.options[@"posY"].isPercent ? [self.options[@"posY"] percentageOf:(float) (screenHeight - NSHeight(self.panel.frame))] : self.options[@"posY"].floatValue;
...
- (void)createTimeout {
  ...
  NSString *text = self.timeout ? [self format:self.options[@"timeout-format"].stringValue withSeconds:(NSUInteger) ceil(self.timeout)] : nil;
...
- (NSString *)debugConstraint:(NSLayoutConstraint *)constraint {
  ...
  return [NSString stringWithFormat:@"%@.%@ %@%@ %@.%@%@%@", firstItem.identifier, [self layoutAttributeToString:constraint.firstAttribute], relation, multiplier, secondItem.identifier, [self layoutAttributeToString:constraint.secondAttribute], constant, priority];
}
...
- (void)processTimer {
  ...
  if (self.timeout > 0.0f) {
    if (self.timeoutLabel != nil) {
      self.timeoutLabel.stringValue = [self format:self.options[@"timeout-format"].stringValue withSeconds:(NSUInteger) ceil(self.timeout)];
    }
  }
  else {
...
#pragma ide diagnostic ignored "-Wdeprecated-declarations"
  if (@available(macOS 10.9, *)) {
    ...
    [alertSheet beginSheetModalForWindow:self.panel modalDelegate:nil didEndSelector:@selector(alertDidEnd:returnCode:contextInfo:) contextInfo:nil];
  }
...
- (void)setLabel:(CDTextField *)label withText:(NSString *)text {
  ...
  CGFloat height = [clone frameForAlignmentRect:rect].size.height;
...
- (void)windowDidResize:(NSNotification *)notification {
  ...
  NSNumber *width = @(self.panel.frame.size.width);
  NSNumber *height = @(self.panel.frame.size.height);
  NSNumber *minWidth = @(self.panel.contentMinSize.width);
  NSNumber *minHeight = @(self.panel.contentMinSize.height);
