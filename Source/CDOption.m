...
- (void)setMaximumValues:(NSNumber *)maximumValues {
  ...
  if (max > 0 && maxIndex > currentMaxIndex) {
    NSInteger missing = maxIndex - currentMaxIndex;

    // Fill missing indices with NSNull (not default value).
    while (missing > 0) {
    ...
    NSInteger extra = currentMaxIndex - maxIndex;
...
- (void)setValue:(id)value atIndex:(NSUInteger)index {
  ...
  _values[index] = [self convertValue:value];
}
...
- (CDOption *(^)(NSUInteger))toValueIndex {
  return ^CDOption *(NSUInteger index) {
    self.deprecatedValueIndex = @(index);
    return self;
  };
}
...
- (NSArray<NSString *> *)arrayOfStrings {
  ...
  for (id item in self.arrayValue) {
    ...
    if ((self.valueType == CDNumber || self.valueType == CDStringOrNumber) && [item isKindOfClass:NSNumber.class] && ((NSNumber *) item).isPercent) {
      NSNumber *number = @(((NSNumber *) item).doubleValue * 100);
...
- (BOOL)hasAutomaticDefaultValue {
  ...
  // accomplished here is to see if it is one and then just assume that it's a
...
- (id)jsonValue {
  ...
  return
    @{
      @"allowedValues": @(self.allowedValues.count),
      @"automaticDefaultValue": @(self.hasAutomaticDefaultValue),
...
- (BOOL)isPercent {
  NSNumber *number = self.numberValue;
  return number != nil && number.isPercent;
}
...
- (NSNumber *)numberValue {
  if (self.valueType == CDStringOrNumber) {
    id value = self.maximumValues.integerValue == 1 ? self.arrayOfStringOrNumbers.lastObject : @(self.arrayOfStringOrNumbers.count);
    if ([value isKindOfClass:NSString.class]) {
    ...
    return self.maximumValues.integerValue == 1 ? self.arrayOfNumbers.lastObject : @(self.arrayOfNumbers.count);
  }
}
