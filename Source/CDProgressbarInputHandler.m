  ...
  for (NSUInteger i = 0; i < lines.count; i++) {
    ...
    if (line.length != 0) {
      if ([line isEqualToString:@"stop enable"]) {
        [self invokeOnMainQueueWithTarget:delegate selector:@selector(stoppable) object:@YES];
      }
      else if ([line isEqualToString:@"stop disable"]) {
        [self invokeOnMainQueueWithTarget:delegate selector:@selector(stoppable) object:@NO];
      }
      else {
...
- (void)main {
  ...
  [self invokeOnMainQueueWithTarget:delegate selector:@selector(progressFinished) object:nil];
}
