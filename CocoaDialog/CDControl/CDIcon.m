...
- (NSImage*) icon {
  ...
  return iconImage  = [self.options hasOpt:@"icon-file"]
                    ? [self iconFromFile:[self.options optValue:@"icon-file"]]
                    : [self.options hasOpt:@"icon"]
                    ? [self iconFromName:[self.options optValue:@"icon"]] : iconImage;
}
-  (NSData*) iconData {
...
- (NSImage*) iconWithDefault {
  ...
  return iconImage = !self.icon ? [NSApp applicationIconImage] : iconImage;
}
...
- (NSImage*) iconFromName:(NSString*)name {
  ...
  NSString *bundle = [self.options hasOpt:@"icon-bundle"]     // Use bundle identifier
  ? [self.options optValue:@"icon-bundle"] : nil,
  *path = nil,
  *iconType = [self.options hasOpt:@"icon-type"]
  ? [self.options optValue:@"icon-type"] : @"icns";
  ...
  if (bundle == nil) {
    ...
    else if ([name caseInsensitiveCompare:@"search"] == NSOrderedSame ||
             [name caseInsensitiveCompare:@"find"] == NSOrderedSame) {
      name = @"Spotlight";
  ...
  if (image == nil) {
    if (bundle != nil || path != nil) {
      ...
      NSString * fileName = !path ? [[NSBundle bundleWithPath:[NSWorkspace.sharedWorkspace absolutePathForAppBundleWithIdentifier:bundle]] pathForResource:name ofType:iconType]
                                  : [[NSBundle bundleWithPath:path] pathForResource:name ofType:iconType];
      if (fileName != nil) {
